...
class Corpus(object):
    ...
    def build_vocabulary(self):
        ...
        discrete_set = set()
        discrete_set_docwords = set()

        for document in self.documents:
            for word in document.words:
                discrete_set_docwords.add(word)
# # 
        filename = open('./texts/app_words/positive-words.txt','r')
        for line in filename.read().split():
            discrete_set.add(line)
 
        filename = open('./texts/app_words/negative-words.txt','r')
        for line in filename.read().split():
            discrete_set.add(line)
        
        list1 = list(discrete_set)
        list2 = list(discrete_set_docwords)
        list3 = [item for item in list1 if item in list2]        
#         self.vocabulary = list(discrete_set)
        self.vocabulary = list3
